...
export function create_mood_fields(fields) {
    ...
    fields.forEach((field, i) => {
        field_els.push(create_mood_field(field, i));
    });

    return field_els;
}

export function create_mood_field(field, index) {
    validate_type(field, AbstractMoodField);
    validate_integer(index, 0);
    let dom_field;
    if(field instanceof NumberField) {
        dom_field = _create_number_field(field);
    } else if(field instanceof TextField) {
        dom_field = _create_text_field(field);
    } else if(field instanceof FractionField) {
        dom_field = _create_fraction_field(field);
    } else if(field instanceof SliderField) {
        dom_field = _create_slider_field(field);
    }
...
export function _create_number_field(field) {
    ...
    const mood_field_wrapper = _create_mood_field_wrapper_with_title(field);
...
function _create_mood_field_wrapper_with_title(field) {
    validate_type(field, AbstractMoodField);
